...
def get_supported_descriptions_for_device(
    device: Any, descriptions: list[Any], platform_type: str
) -> list[Any]:
...
class BasePlatformSetup(Generic[T]):
    ...
    def __init__(
        ...
        create_entity_callback: Callable[
            [NorthTrackerDataUpdateCoordinator, int, Any], T
        ],
    ):
            ...
            for device_id, device in coordinator.data.items():
                if device_id not in added_devices:
                    ...
                    for description in supported_descriptions:
                        entity = self.create_entity_callback(
...
class AdvancedPlatformSetup(BasePlatformSetup[T]):
    ...
    def __init__(
        ...
        """
        super().__init__(
            platform_name, entity_class, entity_descriptions, create_entity_callback
        )
...
def validate_device_name(name: str) -> str:
    ...
    if len(name) > DEVICE_NAME_MAX_LENGTH:
        ...
        LOGGER.debug(
            "Device name truncated from %d to %d characters: '%s' -> '%s'",
            len(name),
            len(truncated_name),
            name,
            truncated_name,
