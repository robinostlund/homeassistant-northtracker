    ...
    async def _async_update_data(self) -> dict[int, NorthTrackerGpsDevice]:
        ...
        try:
            ...
            if not self.api.is_authenticated:
                ...
                username = self.config_entry.data.get(CONF_USERNAME) or self.config_entry.data.get("username") or self.config_entry.data.get("user")
                password = self.config_entry.data.get(CONF_PASSWORD) or self.config_entry.data.get("password")
            ...
            main_devices = [device for device in devices.values() if device.device_type == "gps"]
            if main_devices:
